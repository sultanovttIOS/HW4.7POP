...
class KoronaViewController: UIViewController, TransferProtocol {
    
    private var isRequisiteValid: Bool = false
    private var isSumValid: Bool = false
    private let myBalance = 3000.0
    
    private let requisiteTF = MakerView.shared.makerTf(placeholder: "Введите номер карты", cornerRadius: 12,
    ...
    private lazy var recipientBalanceLabel = MakerView.shared.makerLabel(text: "Баланс: 3000.0 сом",
                                                                         font: UIFont.systemFont(ofSize: 20, weight: .semibold), textColor: .black, numberOfLines: 2)
